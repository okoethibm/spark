...
<tr>
  ...
  <td>
    ...
    where for accessing the Spark master UI through that reverse proxy.
    This is useful when running proxy for authentication e.g. an OAuth proxy. The URL may contain
    a path prefix, like http://mydomain.com/path/to/spark, allowing you to serve the UI for multiple
    Spark clusters and other web applications throguh the same virtual host and port.
    The front-end reverse proxy is responsible for properly rewriting redirects which point
    directly to the Spark master.
    
    <br/>Normally, this should be an absolute URL including scheme (http/https), host and port. 
